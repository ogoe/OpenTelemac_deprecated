...
C           ! l'action conjuguée de la houle et du courant !
C           ! ============================================ !
C
C
...
!
      DO I = 1, NPOIN
!
         IF (TOBW%R(I) > 0.D0) THEN
            TX = TOB%R(I) / (TOB%R(I) + TOBW%R(I))
         ELSEIF (TOB%R(I) > 0.D0) THEN
            TX = 1.D0
         ELSE
            TX = LOCAL_ZERO
         ENDIF
!
         LOGF  = LOG10(2.D0*FW%R(I)/MAX(CF%R(I),1.D-10))
         CSAL  = ABS(COS(ALPHAW%R(I)))
         CSAL1 = CSAL**0.82D0
!        CV A VERIFIER CSAL2 DEVRAIT ETRE EGAL A CSAL1 ???????
