...
C***********************************************************************
C
C      FONCTIONS: MAXIMUM D'UNE VALEUR SUR TOUS LES PROCESSEURS.
C      ==========
C
C-----------------------------------------------------------------------
C                             ARGUMENTS
...
C-----------------------------------------------------------------------
C
      CALL MPI_ALLREDUCE(MYPART,P_DMAX,1,MPI_DOUBLE_PRECISION,MPI_MAX,
     &                   MPI_COMM_WORLD,IER)
C
      IF(IER.NE.0) THEN
        IF(LNG.EQ.1) WRITE(LU,*) 'P_DMAX : ERREUR DANS MPI_ALLREDUCE'
        IF(LNG.EQ.2) WRITE(LU,*) 'P_DMAX: ERROR IN MPI_ALLREDUCE'
